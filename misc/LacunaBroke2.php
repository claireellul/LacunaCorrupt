	...
	<body>
		...
		<script>
			...
			function getattributes () {
				...
				// attributeresponse.forEach( function(tableToDisplay, tableIndex) {
					// tableString = "";
					
					//For each ROW
					// tableToDisplay[2].forEach( function(attributeObject) {
					...
					// attributeArray.forEach ( function(attributeCol, colID) {
						// if (colID === 0) { tableString.concat("<tr>") }
						// tableString = [tableString, "<td>", attributeCol[colID], "</td>"].join();
						// if (colID === attributeArray.length) { tableString.concat("</tr>") }
					// });
					// $("#attributesholder").append(["<div>", attributeresponse[tableIndex][0], "<br><br>", "<table>", tableString, "</table>", "</div>"]);
				});
