<?php 
	...
	function getModel($modelname) {
		...
		if ($modelname == '"Vegetation"') {
			$modelquery = pg_query($db, 'SELECT ST_AsText(geom), "ID" FROM  $modelname LIMIT 5 ; ');
			}
		else {
			$modelquery = pg_query($db, 'SELECT ST_AsText(geom), "ID" FROM  $modelname LIMIT 200 ; ');
			}
		...
		while ($model = pg_fetch_row($modelquery)) {
			...
			if (substr( $model[0], 0, 18  ) === "GEOMETRYCOLLECTION") { 
				...
				if ($geomtype[0] == "POLYHEDRALSURFACE Z") {  $POLYHEDRALSURFACEZexportString = $POLYHEDRALSURFACEZexportString . $geomtype[1] . " %%% "; array_push($polyhedralsurfaceIDID, $model[1]); }
