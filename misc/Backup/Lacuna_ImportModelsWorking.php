	...
	<body>
									...
									if (!$result) {
									  ...
									  exit;
										...
										while ($layer = pg_fetch_row($result)) {
											echo "<br>";
											echo "<input type='checkbox' id='checkbox1' style='vertical-align: middle; float:left; width:45px' />";
											echo "$layer[0]";
											echo "<br>";
										}
									}
		...
		<script>
			...
			function init() {
				...
				renderer.setClearColor( '#1F2829', 1 )
				...
				var modelVertices = [];
				var aLayer = <?php echo getModel('"BuildingRoofs"'); ?>;
				//console.log(aLayer);
				if (aLayer[1][0] == "POLYHEDRALSURFACE Z") {
					//console.log("Hello");
					...
					aLayerArray.forEach( function(arrayModel) {
						...
						modelVertices.forEach(function(part, index, theVertices) {
							...
							if (part.length != 3) { console.log("Vertex list length not eqaul to 3: can't convert to Three.js vertex") }
								...
								// The triangulator doesn't appear to take into accoun the fact that some points may share X Y coordinates, to overcome this we add an epsilon
