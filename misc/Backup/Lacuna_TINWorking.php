	...
	<body>
		...
		<script>
			...
			function init() {
				...
				var material = new THREE.MeshNormalMaterial( {color: 0x27B030, ambient: 0x27B030} );
				var modelVertices = [];
				
				phpLayer = "Vegetation"
				if (phpLayer = "Vegetation") {
				<?php $buildingwalls = '"Vegetation"';?>;
				}
				
				var aLayer = <?php echo getModel($buildingwalls); ?>;
				...
				if (aLayer[1][0] == "POLYHEDRALSURFACE Z" && aLayer[1][1] != null) {
					...
					var material = new THREE.MeshLambertMaterial( {color: 0xD1D481, ambient: 0xD1D481} );
				...
				if (aLayer[2][0] == "TIN Z" && aLayer[2][1] != null) {
					...
					//console.log(aLayerArray);
					//console.log(aLayerArray.length);
					aLayerArray.forEach( function(arrayModel) {
						if (arrayModel != null) {
						arrayModel.slice(0, - 1); //Remove final comma
						arrayModelSplit = arrayModel.split(",");
						arrayModelSplit.pop();
						//console.log(arrayModelSplit);
						arrayModelSplit.forEach( function(modelUnformattedCoords) {
							//if ($.inArray(modelUnformattedCoords, uniqueCoords) === -1) {
								modelCoords = modelUnformattedCoords.split(" ");
								modelVertices.push(modelCoords);
						...
						modelVertices.forEach(function(part, index, theVertices) {
							//console.log(part)
							if ( index+1 % 4 === 0 || part.length != 3) { console.log("Vertex list length not eqaul to 3: can't convert to Three.js vertex") }
							else {
								// The triangulator doesn't appear to take into account the fact that some points may share X Y coordinates, to overcome this we add an epsilon
								// so Three.js thinks they're different coordinates
								// part.forEach(function(coord, i) {
						...
						if (modelVertices != null && modelVertices.length >= 3) {
								numOfTri += 1
								//console.log(triangles[i][0], triangles[i][1], triangles[i][2]);
								modelGeometry.faces.push( new THREE.Face3( 2, 1, 0 ));
								modelGeometry.faces.push( new THREE.Face3( 0, 1, 2 ));
								modelGeometry.faces.push( new THREE.Face3( 1, 0, 2 ));
								modelGeometry.faces.push( new THREE.Face3( 0, 2, 1 ));
								modelGeometry.faces.push( new THREE.Face3( 2, 0, 1 ));
								modelGeometry.faces.push( new THREE.Face3( 1, 2, 0 ));
				...
				var directionalLight1 = new THREE.DirectionalLight(0xFFFFFF, 0.85);
				var directionalLight2 = new THREE.DirectionalLight(0xFFFFFF, 0.85);
				var directionalLight3 = new THREE.DirectionalLight(0xFFFFFF, 0.85);
				var directionalLight4 = new THREE.DirectionalLight(0xFFFFFF, 0.85);
