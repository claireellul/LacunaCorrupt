<?php 
	...
	function getModel($modelname) {
		...
		if ($modelname == '"Vegetation"') {
			$modelquery = pg_query($db, "SELECT ST_AsText(geom), \"ID\" FROM  $modelname ORDER BY \"ID\" LIMIT 10 ; ");
		}
			...
			$modelquery = pg_query($db, "SELECT ST_AsText(geom), \"ID\" FROM  $modelname ORDER BY \"ID\" LIMIT 2000 ; ");
		}
		else {
			$modelquery = pg_query($db, "SELECT ST_AsText(geom), \"ID\" FROM  $modelname ");
		}
